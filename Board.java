package com.MarqusKoryEdwin.Reversi;
...
public class Board {
							...
							for(int i = y; i < 7; i++){ //traversing through line of buttons
								if(boardSquares[x][i].getToolTipText() == "" && i != y){ //check if contains a chip
									...
									for(int y1 = y; y1 < i; y1++){ //switches the chips in between the player chip
										...
										boardSquares[x][y1].setIcon(new ImageIcon( "F:/Othello/Othello/images/Othello Chip " + playerTurn + ".png" ));
									}
							...
							if(boardSquares[x][y].getToolTipText() == ""){
								...
								for(int i = x; i > 0; i--){
									if(boardSquares[i][y].getToolTipText() == "" && i != x){
										...
										for(int x1 = x; x1 > i; x1--){
											...
											boardSquares[x1][y].setIcon(new ImageIcon( "F:/Othello/Othello/images/Othello Chip " + playerTurn + ".png" ));
										}
										switchPlayer = true;
										addPiece(x, y, playerTurn);
										break;
								...
								while(x+(-1*tempCounter) > 0 && y+(-1*tempCounter) > 0){
									if(boardSquares[x+(-1*tempCounter)][y+(-1*tempCounter)].getToolTipText() == ""){
										...
										for(int z = 1; z <= tempCounter; z++){
											...
											boardSquares[x+(-1*z)][y+(-1*z)].setIcon(new ImageIcon( "F:/Othello/Othello/images/Othello Chip " + playerTurn + ".png" ));
										}
										switchPlayer = true;
										addPiece(x, y, playerTurn);
										break;
								...
								if(switchPlayer){
									...
									for(int o = 0; o < 7; o++){
								     ...
								     JOptionPane.showMessageDialog(gui,"\nThe number of white chips there are is " + numWhite );
    ...
    public void addPiece(int posX, int posY, String type){
    	...
    	//checking type
    	if(type == "Black"){
    		tempImage = new ImageIcon("F:/Othello/Othello/images/Othello Chip Black.png");
    		boardSquares[posX][posY].setToolTipText("Black");
    	}else{
    		tempImage = new ImageIcon("F:/Othello/Othello/images/Othello Chip White.png");
